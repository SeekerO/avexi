...
const PDFConverter: React.FC = () => {
  ...
  const allModes: ModeConfig[] = useMemo(
    () => [
      ...
      },
      {
        ...
        accept: ".pdf",
        outputExt: ".xlsx",
        description: "Extract data to Excel",
  ...
  const downloadAllFiles = () =>
    files.forEach((file) => {
      if (file.status === "complete" && file.downloadUrl) downloadFile(file);
    });
  ...
  const handleConvert = async () => {
    ...
    if (currentMode.multiple) {
      ...
      try {
        ...
        const outputName =
          mode === "combine-pdf" ? "combined.pdf" : "images.pdf";
        setFiles([
          {
            id: "combined-result",
  ...
  return (
    <div className="min-h-screen w-full overflow-y-auto transition-colors duration-500 bg-slate-50 dark:bg-[#070710]">
      ...
      <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
        <div
          className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full opacity-10 dark:opacity-15 blur-[100px]"
          style={{
            background:
              "radial-gradient(circle at 100% 0%, rgba(99,102,241,0.5) 0%, transparent 70%)",
          }}
        />
        <div
          className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full opacity-5 dark:opacity-10 blur-[80px]"
          style={{
            background:
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-12 pb-24">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-white/30">
            Professional client-side document processing
          </p>

          <div className="flex items-center justify-center gap-3 mt-4">
            <div
              className={`w-2 h-2 rounded-full ${pdfJsLoaded ? "bg-emerald-500" : "bg-amber-500 animate-pulse"}`}
            />
        ...
        {files.length > 0 && files.every((f) => f.status === "ready") && (
          ...
          <CreditGate toolId={TOOL_ID}>
            {({ onAction, hasCredits, isUnlimited, loading }) => (
              <button
                onClick={() => onAction(handleConvert)}
                disabled={
        ...
        <div className="mt-12 text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            <Image
              ...
              alt="Avexi"
              width={18}
              className="opacity-50 grayscale dark:grayscale-0 dark:opacity-100"
            />
            <span className="text-[10px] font-bold text-slate-400 dark:text-white/20 tracking-[0.3em] uppercase">
              Avexi Studio
            </span>
          </div>
