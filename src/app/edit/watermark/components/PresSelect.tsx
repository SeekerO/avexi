// src/app/edit/watermark/components/PresSelect.tsx
// Presets are filtered based on the user's allowedPresets from Firebase RTDB.
// Admin-assigned presets only are shown. All visible if allowedPresets is undefined (legacy).
...
// ── Preset catalog — IDs must match WATERMARK_PRESETS in UserRolesModal.tsx ──
const ALL_PRESETS = [
  {
    id: "white-eid",
    url: WhiteLogoEID.src,
    name: "White EID Logo",
    type: "logo" as const,
  },
  {
    id: "black-eid",
    url: BlackLogoEID.src,
    name: "Black EID Logo",
    type: "logo" as const,
  },
  {
    id: "white-eid-com",
    url: WHITELogoEIDandCOMELEC.src,
    name: "White EID & COMELEC",
    type: "logo" as const,
  },
  {
    id: "black-eid-com",
    url: BlackLogoEIDandCOMELEC.src,
    name: "Black EID & COMELEC",
    type: "logo" as const,
  },
  {
    id: "comelec",
  ...
  { id: "kkk", url: KKKlogo.src, name: "KKK Logo", type: "logo" as const },
  {
    id: "black-shadow",
    url: BlackShadow.src,
    name: "Black Shadow Footer",
    type: "footer" as const,
  },
  {
    id: "white-shadow",
    url: WhiteShadow.src,
    name: "White Shadow Footer",
    type: "footer" as const,
  },
];
...
function PresetCard({
  ...
  item: (typeof ALL_PRESETS)[number];
  isSelected: boolean;
  onSelect: () => void;
}) {
  return (
...
const PresSelect = () => {
  ...
  // ── FIX: Read allowedPresets directly from Firebase RTDB ─────────────────
  // The Firebase Auth user object only holds JWT claims and never includes
  // custom fields like allowedPresets. We must read them from the RTDB node.
  const [allowedPresets, setAllowedPresets] = useState<string[] | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    if (!user?.uid) return;
    const presetsRef = ref(db, `users/${user.uid}/allowedPresets`);
    const unsub = onValue(presetsRef, (snap) => {
      const val = snap.val();
      // Firebase drops empty arrays and returns null — we use a "__none__"
      // sentinel written by the admin panel to distinguish:
      //   null            → field never set (legacy) → show all
      //   ["__none__"]    → explicitly set to empty  → show nothing
      //   ["id1", ...]    → explicit allow-list
      if (val === null) {
        setAllowedPresets(undefined); // legacy / no restriction
      } else if (Array.isArray(val) && val[0] === "__none__") {
        setAllowedPresets([]); // explicitly empty
      } else {
        setAllowedPresets(Array.isArray(val) ? val : undefined);
      }
  ...
  // Filter presets: undefined means no restriction → show all (legacy / admin)
  const visiblePresets = useMemo(() => {
    if (!presetsLoaded) return [];
    if (allowedPresets === undefined) return ALL_PRESETS; // no restriction
    if (allowedPresets.length === 0) return []; // explicitly empty
    return ALL_PRESETS.filter((p) => allowedPresets.includes(p.id));
  }, [allowedPresets, presetsLoaded]);
  ...
  const selectedPreset = useMemo(
    () => visiblePresets.find((img) => img.url === selectedImageUrl) ?? null,
    [selectedImageUrl, visiblePresets],
  );
  ...
  const handleAddAsLogo = useCallback(() => {
    ...
    else addGlobalLogo(selectedPreset.url);
    closeModal();
  }, [
    selectedPreset,
    isIndividual,
  ...
  const handleAddAsFooter = useCallback(() => {
    ...
    else addGlobalFooter(selectedPreset.url);
    closeModal();
  }, [
    selectedPreset,
    isIndividual,
  ...
  const logos = visiblePresets.filter((i) => i.type === "logo");
  const footers = visiblePresets.filter((i) => i.type === "footer");

  // Still loading from RTDB
  if (!presetsLoaded) {
    return (
      <div className="w-full mb-20 flex items-center justify-center gap-2 p-3.5 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
  ...
  if (visiblePresets.length === 0) {
    return (
      <div className="w-full mb-20 flex items-center justify-center gap-2 p-3.5 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
        <p className="text-xs text-gray-400 dark:text-gray-500">
          No presets assigned to your account
        </p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {showModal && (
        ...
        >
          <div className="relative -mt-40 w-full max-w-2xl h-[60%] flex flex-col rounded-2xl overflow-hidden shadow-2xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  ...
                  {visiblePresets.length} available
                </p>
              </div>
              <button
                onClick={closeModal}
              ...
              {selectedPreset ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    >
                      <Layers className="w-3.5 h-3.5" /> Footer
                    </button>
                  </div>
                </div>
              ) : (
