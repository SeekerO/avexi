...
import {
  ImageEditorProvider,
  useImageEditor,
} from "./components/ImageEditorContext";
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                ...
                setSnapIndex(next);
                if (sheetRef.current)
                  sheetRef.current.style.height = `${snapPoints[next]}%`;
              }}
            ...
            >
              <ChevronDown
                className={`w-4 h-4 transition-transform ${snapIndex === snapPoints.length - 1 ? "" : "rotate-180"}`}
              />
            </button>
            <button
              onClick={onClose}
...
function WatermarkPageContent() {
  ...
  const adjustCount = Object.values(activeAdjustments).filter(
    (v) => v !== 0,
  ).length;
  ...
  const TABS: {
    ...
    },
    {
      ...
      label: "Metadata",
      icon: Tag,
      count: metadata.enabled ? 1 : undefined,
  ...
  const renderTabContent = () => (
    <>
            ...
            <div className="space-y-10 ">
              <div className="p-3 bg-violet-50 dark:bg-violet-500/10 border border-violet-200 dark:border-violet-500/20 rounded-lg">
                <p className="text-xs text-violet-700 dark:text-violet-300">
                  💡 Switch to <strong>Upload</strong> to add logos and footers,
                  then control them here.
                </p>
              </div>
              <ImageControls />
            </div>
      ...
      {activeTab === "metadata" && (
        <MetadataInjector metadata={metadata} onChange={setMetadata} />
      )}
    </>
  );
  ...
  return (
    <div className="min-h-full overflow-y-auto font-sans w-full bg-gray-50 dark:bg-gray-950">
      ...
      <div className="hidden lg:flex h-screen overflow-hidden w-full">
        {/* ── Editor Sidebar ── */}
        <div
          className="w-[320px] flex-shrink-0 h-screen flex flex-col
                        bg-white dark:bg-[#0d0d1a]
        ...
        >
          ...
          <div
            className="pointer-events-none absolute top-0 right-0 w-64 h-64 rounded-full opacity-0 dark:opacity-30"
            style={{
              background:
          ...
          />
          <div
            className="pointer-events-none absolute bottom-0 left-0 w-48 h-48 rounded-full opacity-0 dark:opacity-20"
            style={{
              background:
