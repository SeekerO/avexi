...
export default function LogoMaker() {
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    if (isDragging && selected && !isResizing) {
      ...
      updateEl(selected, {
        width: Math.max(
          20,
          resizeStart.w + (e.clientX - resizeStart.x) / scale,
        ),
        height: Math.max(
          20,
          resizeStart.h + (e.clientY - resizeStart.y) / scale,
        ),
      });
    }
  };
  ...
  const downloadCanvas = async (withBg: boolean) => {
    ...
    for (const el of sorted) {
      ...
      if (el.type === "text") {
        ...
        if (el.content === "Circle") ctx.arc(cx, cy, r, 0, Math.PI * 2);
          ...
          drawTriangle(ctx, el.x, el.y, el.width, el.height);
        ctx.fill();
    ...
    canvas.toBlob((blob) => {
      ...
      a.href = url;
      a.download = `logo-${withBg ? "with-bg" : "no-bg"}.png`;
      a.click();
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-sm font-semibold text-gray-700 dark:text-white/80">
            ...
            <h1
              className="font-syne text-lg font-extrabold tracking-tight text-slate-800 dark:text-transparent dark:bg-clip-text"
              style={{
                backgroundImage: "linear-gradient(90deg,#f9fafb,#9ca3af)",
              }}
            >
              Logo Maker
            </h1>
          ...
          <div className="flex items-center gap-0.5">
            <ToolBtn onClick={undo} disabled={historyStep <= 0} title="Undo">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            {showCanvasSettings && (
              <div
                className="absolute right-0 top-full mt-2 z-50 w-72 rounded-xl
                ...
                <div className="space-y-1 mb-3">
                  {PRESETS.map((p) => (
                    ...
                    >
                      ...
                      <span className="text-gray-400 dark:text-white/25 font-mono">
                        {p.width} × {p.height}
                      </span>
                    </button>
                  ))}
          ...
          <div
            ...
            border border-black/[0.08] dark:border-white/[0.08]"
          >
            <ToolBtn onClick={() => handleZoom(1)}>
              <ZoomOut className="w-3.5 h-3.5" />
            </ToolBtn>
            <span className="text-xs font-mono font-medium w-10 text-center text-gray-600 dark:text-white/50">
              {zoom}%
            </span>
            <ToolBtn onClick={() => handleZoom(-1)}>
          ...
          <CreditGate toolId={TOOL_ID}>
            {({ onAction, hasCredits, isUnlimited, loading }) => (
              <button
                onClick={() => onAction(() => downloadCanvas(true))} // Changed this line
          ...
          <div className="p-4 space-y-5">
            ...
            <div>
              ...
              <label className="flex items-center gap-2 cursor-pointer mb-2">
                ...
                />
                <span className="text-xs text-gray-600 dark:text-white/45">
            ...
            <div className="p-4 space-y-5">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-0.5">
                  <ToolBtn onClick={duplicateEl} title="Duplicate">
                ...
                <div className="grid grid-cols-6 gap-1.5">
                  {COLORS.map((c) => (
                    <ColorSwatch
                      key={c}
                      color={c}
                      active={selectedEl.color === c}
                      onClick={() => {
                        updateEl(selectedEl.id, { color: c });
                        pushHistory(elements);
                      }}
                    />
                  ))}
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateEl(selectedEl.id, { fontSize: +e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      <ToolBtn
                        active={selectedEl.fontWeight === "bold"}
                        onClick={() => {
                  ...
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      {(["left", "center", "right"] as const).map((a) => (
                        <ToolBtn
                          key={a}
                          active={selectedEl.textAlign === a}
                          onClick={() => {
                            updateEl(selectedEl.id, { textAlign: a });
                            pushHistory(elements);
                          }}
                        >
                          {a === "left" && (
              ...
              <div>
                <SectionLabel>
                  Size — {Math.round(selectedEl.width)} ×{" "}
                  {Math.round(selectedEl.height)}
                </SectionLabel>
                <div className="space-y-2">
                  {(["width", "height"] as const).map((k) => (
                    <div key={k}>
                      <p className="text-[10px] text-gray-400 dark:text-white/25 mb-1 capitalize">
                        {k}
                      </p>
                      <input
                        type="range"
                        min={20}
                        max={k === "width" ? canvasW : canvasH}
                        value={selectedEl[k]}
                        onChange={(e) =>
                          updateEl(selectedEl.id, { [k]: +e.target.value })
                        }
                        onMouseUp={() => pushHistory(elements)}
                        className="w-full accent-indigo-500"
                      />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateEl(selectedEl.id, { rotation: +e.target.value })
                  }
                  onMouseUp={() => pushHistory(elements)}
                  className="w-full accent-indigo-500"
                />
              ...
              <div>
                ...
                <div className="flex gap-1.5">
                  ...
                  <button
                    disabled={selectedEl.zIndex === elements.length - 1}
                    onClick={() => {
                      updateEl(selectedEl.id, {
                        zIndex: selectedEl.zIndex + 1,
                      });
